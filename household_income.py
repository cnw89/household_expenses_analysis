...
INDIR = r'./'
filename = INDIR + 'hdi_composition_all.xlsx'
...
#1 merge retired and non-retired
df_comp.loc['1 adult without children'] += df_comp.loc['1 adult']
df_comp.loc['2 adults without children'] += df_comp.loc['2 or more adults']

df_comp.drop(index=['1 adult', '2 or more adults'], inplace=True)

#2 express percentiles as per ALL households, rather than decile composition
df_comp = df_comp/100
for col in df_comp:
...
eff_adults_3_or_more_adults_no_children = 3.2
eff_children_1_adult_with_children = 1.5
eff_children_2_adults_with_3_or_more_children = 3.2
eff_adults_3_or_more_adults_with_children = 3.2
eff_children_3_or_more_adults_with_children = 1.5
...
f_pcInd_to_required_incomesum = interp1d(np.asarray(required_incomesum.index), np.asarray(required_incomesum))
